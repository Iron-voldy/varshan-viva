...
$reports_query = "SELECT m.record_id, p.first_name, p.last_name, m.diagnosis, m.treatment_plan, m.prescribed_medications, m.follow_up_date, m.visit_date 
                  FROM MedicalRecords m 
                  JOIN Patients p ON m.patient_id = p.patient_id 
                  WHERE m.doctor_id = ?";
$stmt = Database::$connection->prepare($reports_query);
...
<head>
    ...
    <title>Manage Medical Reports - Doctor Dashboard</title>
...
<div class="container">
    <h1>Manage Medical Reports</h1>

    <h3>Add New Medical Report</h3>
    <form method="POST" action="add-medical-report.php">
    ...
    <table>
        ...
        <tr>
            ...
            <td><?= htmlspecialchars($report['treatment_plan']) ?></td>
            <td><?= htmlspecialchars($report['prescribed_medications']) ?></td>
            <td><?= $report['follow_up_date'] ? date('Y-m-d', strtotime($report['follow_up_date'])) : 'N/A' ?></td>
            <td>
                <a href="edit-medical-report.php?id=<?= $report['record_id'] ?>" class="btn btn-primary">Edit</a>
                <a href="delete-medical-report.php?id=<?= $report['record_id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
            </td>
        </tr>
        <?php endforeach; ?>
    </table>
